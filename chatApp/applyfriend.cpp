    ...
    ui(new Ui::ApplyFriend),_label_point(2,6)
{
    ...
    this->setModal(true);
    ui->name_ed->setPlaceholderText(tr("Chis"));
    ui->lb_ed->setPlaceholderText("Search");
    ui->name_ed->setPlaceholderText("XD");
...
void ApplyFriend::InitTipLbs()
{
    ...
    for(int i = 0; i < _tip_data.size(); i++){
        ...
        int textWidth = fm.horizontalAdvance(lb->text());
...
void ApplyFriend::SlotApplySure()
{
    ...
    //发送tcp请求给chat server
    emit TcpMgr::GetInstance()->sigSendData(ReqId::ID_ADD_FRIEND_REQ, jsonString.toUtf8());
    this->hide();
    deleteLater();
}
