...
void ChatPage::SetUserInfo(std::shared_ptr<UserInfo> user_info)
{
    ...
    ui->chat_msg_list->removeAllItem();
...
void ChatPage::on_send_btn_clicked()
{
    ...
    {
        //消息内容长度不合规就跳过
        if(msgList[i].contents.length() > 1024){
            continue;
        }
        ...
        if(type == "text")
        {
            ...
            if(txt_size + msgList[i].contents.length()> 1024){
            ...
            txt_size += msgList[i].contents.length();
            QJsonObject obj;
            QByteArray utf8Message = msgList[i].contents.toUtf8();
