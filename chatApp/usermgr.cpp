...
UserMgr::UserMgr():m_user_info(nullptr),_chat_loaded(0),_contact_loaded(0)
{
...
void UserMgr::AppendApplyList(QJsonArray jsonArry)
{
    for(const QJsonValue &value: jsonArry)
    {
...
void UserMgr::AppendFriendList(QJsonArray jsonArry)
{
    for(const QJsonValue &value: jsonArry)
    {
        auto name = value["name"].toString();
        ...
        auto info = std::make_shared<FriendInfo>(  uid,  name,  nick,  icon,
                                                 sex,  desc,  back);
